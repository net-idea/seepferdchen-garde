(function () {
  ...
  // Only inject a success alert if the server did not render a full summary
  const hasServerSummary = !!document.getElementById('booking-success-summary');

  if (params.has('sent')) {
    if (!hasServerSummary) {
      showAlert(
        successEl,
        'Vielen Dank! Ihre Anmeldung ist eingegangen. Bitte bestätigen Sie diese über den Link in der E‑Mail.',
        'alert-success'
      );
    }
    ...
    if (e === 'rate') msg = 'Bitte einen Moment warten, bevor das Formular erneut abgesendet wird.';
  ...
  if (params.has('sent') || params.has('error')) {
    const hash = window.location.hash || (params.has('sent') ? '#booking-success' : '#booking-error');
